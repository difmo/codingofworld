        ...
        />
          ...
          {pricingData.map((pricing) => (
            <PricingBox
              ...
              price={pricing.price}
              duration={isMonthly ? pricing.monthly : pricing.yearly}
              subtitle={pricing.subtitle}
            >
              {Array.isArray(pricing.offers) ? pricing.offers : Object.values(pricing.offers).map((offer, index) => (
                <OfferList key={1} text={offer.text} status={offer.status} />
              ))}
              <button onClick={() => handleEdit(pricing.id)} className="mt-3 bg-blue-500 text-white py-2 px-4 rounded">
                Edit
              </button>
              <button onClick={() => handleDelete(pricing.id)} className="mt-3 bg-red-500 text-white py-2 px-4 rounded">
                Delete
              </button>
            </PricingBox>
          ))}
