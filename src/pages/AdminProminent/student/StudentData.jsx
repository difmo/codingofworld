        ...
        <div className="w-full overflow-x-auto">
          <table className="min-w-full border border-collapse border-gray-300">
            <thead className="text-white bg-green-600">
              <tr>
                {columnVisibility.name && <th className="w-10 px-4 py-2 border">Sr No.</th>}
                {columnVisibility.name && <th className="w-48 px-4 py-2 border">Student Name</th>}
                {columnVisibility.email && <th className="w-48 px-4 py-2 border">Email</th>}
                {columnVisibility.college && <th className="w-48 px-4 py-2 border">College</th>}
                {columnVisibility.qualification && <th className="w-48 px-4 py-2 border">Qualification</th>}
                {columnVisibility.internshipType && <th className="w-48 px-4 py-2 border">Internship Type</th>}
                {columnVisibility.mobile && <th className="w-48 px-4 py-2 border">Mobile</th>}
                {columnVisibility.createdAt && <th className="w-48 px-4 py-2 border">Date</th>}
                {columnVisibility.resumeURL && <th className="w-48 px-4 py-2 border">Resume URL</th>}
                {columnVisibility.actions && <th className="w-20 px-4 py-2 border">Actions</th>}
              </tr>
            </thead>
            <tbody>
              {studentData.map((student, index) => (
                <tr key={student.id} className="hover:bg-gray-100">
                  {columnVisibility.name && <td className="px-4 py-2 border">{index + 1}</td>}
                  {columnVisibility.name && <td className="px-4 py-2 border">{student.name}</td>}
                  {columnVisibility.email && <td className="px-4 py-2 border">{student.email}</td>}
                  {columnVisibility.college && <td className="px-4 py-2 border">{student.college}</td>}
                  {columnVisibility.qualification && <td className="px-4 py-2 border">{student.qualification}</td>}
                  {columnVisibility.internshipType && <td className="px-4 py-2 border">{student.internshipType}</td>}
                  {columnVisibility.mobile && <td className="px-4 py-2 border">{student.mobile}</td>}
                  {columnVisibility.createdAt && <td className="px-4 py-2 border">{student.createdAt}</td>}
