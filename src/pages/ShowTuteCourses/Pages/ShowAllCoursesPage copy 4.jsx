...
const migrateWithSubTopicsCourseIds = async () => {
  try {
   ...
   const migrateSingleCourseWithSubTopics = async (courseId) => {
  try {
    const auth = getAuth();
    const currentUserId = auth.currentUser ? auth.currentUser.uid : null;
    const currentUserEmail = "pritamcodeservir@gmail.com";
    ...
    if (!courseData.title) {
      console.error("Course has no title, skipping migration.");
      return;
    }

    const newCourseId = slugify(courseData.title);
    const newCourseRef = doc(db, "tutorial-courses", newCourseId);
    ...
    // Migrate topics
    const topicsSnapshot = await getDocs(collection(courseRef, "topics"));
    for (const topicDoc of topicsSnapshot.docs) {
      const topicData = topicDoc.data();
      const newTopicRef = doc(newCourseRef, "topics", topicDoc.id);
      await setDoc(newTopicRef, topicData);
    ...
    console.log(`Course "${courseId}" migrated successfully to 'tutorial-courses'.`);
  } catch (error) {
    ...
    // migrateSingleCourseWithSubTopics("7b08e1fb-61ee-4f75-b28d-f79d472f985b");
